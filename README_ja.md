...
フロントエンド開発時は同じdirectoryで `npm run dev` を使用します。Go viewerはreview data、raw diff itemとMarkdown断片、質問の永続化、質問APIを担当します。Reactはdocumentの描画、browser上の操作、シンタックスハイライト、Lucide iconを担当します。`react_render.go` が両者のbootstrap境界であり、Goからは描画済みSVGではなくfile statusやcategory nameなどのicon semanticsを渡します。
