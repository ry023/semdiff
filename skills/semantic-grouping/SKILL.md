...
Group と Step の `summary` は、変更の理由・内容・効果を Markdown の箇条書きで伝える説明です。

#### 原則

各箇条書き項目では一つの主張を述べます。子の箇条書きは、親の理由、具体的な内容、結果、依存関係だけを補足します。通常は入れ子を一段までとし、二段目は依存関係を明確にする場合だけ使います。項目数を揃えるために内容を増やさないでください。

原則として、次の順番で説明します。

- Why: 変更前のどの不足・制約が、今回の変更を必要にするか。
- What: 何をどう変えるか。理解に必要な識別子の種別と新規／既存を示す。
...
「古い implementation を command layer に置き換え、cache と test を追加する」のように、複数の変更を一つの項目へ詰め込まないでください。

### 認知単位と同一ルールの反復変更

#### 位置付け

Step は、レビュアーが一度に理解する認知単位です。Fragment は、その理解を支える具体的な変更の単位です。分割では、一単位の複雑さと、単位数が増えることで生じる移動・一覧把握の負荷を合わせて考えます。

#### 原則

Step や Fragment の件数、行数に固定の目安は設けません。分割によって一単位の理解が容易になる度合いと、分割によって増える移動・一覧把握の負荷を比較します。

同じ目的から生じ、同じ変換ルールを適用し、適用箇所ごとに独立したレビュー判断を必要としない反復変更は、一つの認知単位にまとめます。見た目や構文が似ているだけでは、この条件を満たしません。例外的な処理や別の意味変更を含む箇所は、独立して判断できる単位へ分けます。

同一ファイル内の反復変更は、必要に応じて複数の range を持つ一つの Fragment にまとめます。複数ファイルにまたがる場合は、ファイルごとに Fragment を作り、それらを一つの Step にまとめます。description が対象名以外ほぼ同じになることは、同一ルールの反復適用である可能性を示す手掛かりです。

#### 例

- 同じ規則で変数名の suffix を変更する場合、同一ファイル内の変更は一つの複数 range Fragment にまとめます。複数ファイルにまたがる場合は、ファイルごとの Fragment を一つの Step にまとめます。
- ファイル名の変更に追随して複数ファイルの import path を更新する場合、import 更新はファイルごとの Fragment とし、一つの Step にまとめます。循環依存を避けるために package 構造も変更した箇所は、別の判断が必要なため分けます。
...
Group の title と summary は、一つの成果について何を判断するのかを示します。

#### 原則

title は「保存済みレコードを ID で取得できるようにする」のように、具体的な成果を動作の表現で示します。「implementation」「tests」「backend」「cleanup」のような広い分類や、抽象的な名詞句は使いません。

summary は成果全体の Why・What・So what を説明します。すべての Fragment を言い換えて列挙せず、それらが一つの成果をどう成立させるかを示します。
...
一つの Step を読み終えた時点で、レビュアーが一つの具体的な結論を言えるようにします。複数の独立した問い、処理経路、前提を同時に保持しなければ説明できない場合は分割を検討します。隣接する Step が同じ前提・問い・結論を共有する場合は統合を検討します。

分割によって減る Step 内の理解負荷が、Step 数の増加による移動・一覧把握の負荷より大きい場合にだけ分割します。同一ルールを複数箇所へ適用する反復変更は、適用箇所ごとに独立した判断が必要でない限り、一つの Step にまとめます。
...
Step の title と summary は、Group を理解するための一段階を示します。

#### 原則

title は、その段階で何ができるようになるか、または何を実装するかを動作の表現で示します。ファイル名、層、テスト分類、一般的な工程、読解の指示は使いません。

summary は、その段階の具体的な変更と、前後の Step との関係を説明します。PR 全体の動機や Group の Why を各 Step で繰り返さないでください。単一 Step の Group でも Group summary を全文複製せず、Step では具体的な変更とその効果だけを短く示します。
...
Fragment は Step より細かな証拠単位であり、必ずしも一つずつを認知上の一口サイズにする必要はありません。同じ目的と変換ルールによる同一ファイル内の反復変更は、適用箇所ごとに独立した意味がなければ、一つの複数 range Fragment にまとめます。
...
- 最大の Step が一つの結論として理解できるか確認し、複数の独立した問いを同時に保持する必要があれば分割します。
- 小さい隣接 Step が同じ前提・問い・結論を共有していないか確認し、共有していれば統合します。
- 同一ルールの反復変更が適用箇所ごとに細分化されていないか確認し、独立した判断が不要なら一つの Fragment または Step にまとめます。
