...
1. Run `semdiff grouping init --json` to create a resumable draft containing the exact range, Git-derived suggestions, and mechanical category suggestions. With no range, the CLI uses the current pull request's base branch when `gh` can identify one, otherwise the repository's default branch; it compares the merge base with `HEAD` so unrelated updates to the base branch are excluded. Pass `<base>..<head>` only when the user requests a particular range or automatic detection cannot represent the intended review. Suggestions are navigation aids, not fragments that must each be assigned.
2. Run `semdiff commits <base-sha>..<head-sha> --json` with the SHAs returned by `grouping init` to understand the change narrative. Do not load the complete diff up front.
