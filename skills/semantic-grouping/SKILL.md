...
4. Run `semdiff grouping inspect --suggestions --json` and review suggestions by file and neighboring code. Before naming Groups, inventory the independently reviewable outcomes across the whole range; do not settle the Group structure from the first files, commits, or suggestions inspected. For a seeded draft, prioritize ungrouped suggestions and Groups whose files changed after the source review. Use `semdiff show --draft .semdiff/grouping-draft.json <id> --json` for the relevant candidates.
...
A Group is a collection of Fragments that together support one meaningful review verdict about an outcome of the PR. Discover these outcomes from behavior, contracts, data flow, configuration effects, migrations, and separately motivated cleanup—not from the amount or layout of the diff. One file can contain several Groups when it implements independent outcomes; dozens of files can remain one Group when they jointly deliver one outcome. Neither file count, line count, commit count, nor suggestion count is a proxy for Group count.

Start with each independently reviewable outcome as a candidate Group. Merge candidates when they do not support distinct verdicts and reviewing either one alone would be incomplete or misleading. Split a candidate when at least one of these is true:

- its parts can reasonably receive different review verdicts or be reverted independently;
- it changes distinct externally observable behaviors, contracts, data migrations, or configuration effects with different failure modes;
- understanding one part requires domain context that the other part does not;
- its title or summary can stay accurate only by joining separate outcomes with wording such as “also,” “unrelated,” or “in addition.”

Do not split solely because changes are in different files, packages, architectural layers, commits, or file categories. Keep an implementation with the tests, documentation, generated output, caller adaptations, and configuration required to establish that same outcome unless one of those changes has its own review verdict.

Before finalizing, audit the actual boundaries without aiming for a count:

- For the largest Group, ask whether all Fragments support the same verdict; split it if they do not.
- For each small or adjacent pair of Groups, ask whether they really support distinct verdicts; merge them when they do not.
- Check that titles name concrete outcomes rather than broad buckets such as “implementation,” “tests,” “backend,” or “cleanup.”
- Compare the resulting Groups with the full outcome inventory so early inspection order has not hidden later concerns.

## Review Step boundaries

A Step is one ordered stage in understanding and judging its Group, not a quota and not a smaller Group. Use one Step when all of the Group's Fragments can be explained under one concrete action and one review question, even if that Step spans several files. A single-file, single-purpose change will often have one Group with one Step.

Create another Step only when it introduces a distinct prerequisite, implementation transition, or correctness question that is useful before the next Step. Do not create one Step per Fragment merely to distribute the inventory. Merge adjacent Steps when they answer the same question or when either title is merely a file, layer, test category, or generic phase. Conversely, a large Group may have as many Steps as its real dependency path requires. Step count therefore scales with the internal reasoning needed for that Group, not directly with Group count or PR size.
