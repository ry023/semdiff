...
document.querySelectorAll('.main-group').forEach(function(group){var summary=group.querySelector(':scope > summary');var actions=summary.querySelector('.group-actions')||summary;makeTarget({type:'group',group_id:group.dataset.groupId},summary,actions,'group-ask');group.querySelectorAll('.fragment-note').forEach(function(note){var id=(note.querySelector('.fragment-note-id')&&note.querySelector('.fragment-note-id').textContent.split(' · ')[0].trim())||'';if(!id)return;makeTarget({type:'fragment',group_id:group.dataset.groupId,fragment_id:id},note.closest('pre'),note,'fragment-ask')})});
