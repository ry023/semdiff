...
document.querySelectorAll('.guided-group').forEach(function(group){
  ...
  group.querySelectorAll('.review-step').forEach(function(step){var summary=step.querySelector(':scope > summary');targets.set(key({type:'step',group_id:group.dataset.groupId,step_id:step.dataset.stepId}),summary);step.querySelectorAll('.fragment-note').forEach(function(note){var id=(note.querySelector('.fragment-note-id')&&note.querySelector('.fragment-note-id').textContent.split(' · ')[0].trim())||'';if(id)targets.set(key({type:'fragment',group_id:group.dataset.groupId,fragment_id:id}),note.closest('.guided-file')||note.closest('pre'))})})
});
