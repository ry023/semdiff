...
Run `npm run dev` in the same directory for frontend development. The Go viewer owns review data, raw Markdown strings, structured diff items, question persistence, and the question API. React owns the rendered document, Markdown rendering, browser interaction, syntax highlighting, and Lucide icons. `react_render.go` is the bootstrap boundary between them; Go passes icon semantics such as file status and category name rather than rendered SVG.
